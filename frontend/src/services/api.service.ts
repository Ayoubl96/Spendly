...
class ApiService {
  ...
  private mapExpenseFields(expense: any): Expense {
    return {
      ...
      userShareAmount: expense.user_share_amount
        ? parseFloat(expense.user_share_amount)
        : undefined,
      userSharePercentage: expense.user_share_percentage
        ? parseFloat(expense.user_share_percentage)
        : undefined,
      expenseShares: expense.expense_shares
        ? expense.expense_shares.map((share: any) => ({
            id: share.id,
            ...
            customAmount: share.custom_amount
              ? parseFloat(share.custom_amount)
              : undefined,
  ...
  async getExpenses(
    ...
    const rawExpenses = await this.request<any[]>(
  ...
  async getExpenseSummary(
    ...
    return this.request<ExpenseSummary>(
      `/expenses/summary/monthly?year=${year}&month=${month}`,
    );
  }
  ...
  async reorderCategories(
    ...
    return this.request<{ message: string; updated_count: number }>(
      "/categories/reorder",
      {
        method: "POST",
  ...
  async getBudgetGroupsSummary(includeInactive = false): Promise<any> {
    return this.request(
      `/budget-groups/summary?include_inactive=${includeInactive}`,
    );
  }

  async createBudgetGroup(
    data: CreateBudgetGroupRequest,
  ): Promise<BudgetGroup> {
  ...
  async getBudgetGroupWithBudgets(
    budgetGroupId: string,
  ): Promise<BudgetGroupWithBudgets> {
    return this.request<BudgetGroupWithBudgets>(
      `/budget-groups/${budgetGroupId}/with-budgets`,
    );
  }

  async getBudgetGroupSummary(
    budgetGroupId: string,
  ): Promise<BudgetGroupSummary> {
    return this.request<BudgetGroupSummary>(
      `/budget-groups/${budgetGroupId}/summary`,
    );
  }
  ...
  async generateBudgetsForGroup(
    budgetGroupId: string,
    data: GenerateBudgetsRequest,
  ): Promise<{ created: number }> {
    return this.request<{ created: number }>(
      `/budget-groups/${budgetGroupId}/generate-budgets`,
      {
        method: "POST",
        body: JSON.stringify({
          category_scope: data.category_scope ?? "all",
          ...
          include_inactive_categories:
            data.include_inactive_categories ?? false,
        }),
      },
    );
  }
  ...
  async bulkUpdateBudgets(
    budgetGroupId: string,
    data: BulkBudgetsUpdateRequest,
  ): Promise<{ updated: number }> {
    return this.request<{ updated: number }>(
      `/budget-groups/${budgetGroupId}/bulk-update-budgets`,
      {
        method: "POST",
        body: JSON.stringify(data),
      },
  ...
  async getAnalyticsSummary(
    year: number,
  ): Promise<{ yearly_expenses: any; budget_performance: BudgetSummary }> {
  ...
  async commitExpenseImport(
    data: ExpenseImportCommitRequest,
  ...
  async getCategorizationRules(
    ...
    return this.request<CategorizationRule[]>(
      `/expense-import/rules${params.toString() ? `?${params.toString()}` : ""}`,
    );
  }
  ...
  async getPaymentMethods(includeInactive?: boolean): Promise<PaymentMethod[]> {
    ...
    const rawPaymentMethods = await this.request<any[]>(
      `/payment-methods/${params}`,
    );
    return rawPaymentMethods.map((pm) => this.mapPaymentMethodFields(pm));
  }

  async getPaymentMethodsWithStats(
    includeInactive?: boolean,
  ): Promise<PaymentMethodWithStats[]> {
    const params = includeInactive ? "?include_inactive=true" : "";
    const rawPaymentMethods = await this.request<any[]>(
      `/payment-methods/with-stats${params}`,
    );
    return rawPaymentMethods.map((pm) =>
      this.mapPaymentMethodWithStatsFields(pm),
    );
  }
  ...
  async reorderPaymentMethods(
    data: BulkPaymentMethodUpdateRequest,
  ): Promise<PaymentMethod[]> {
    const rawPaymentMethods = await this.request<any[]>(
  ...
  async createDefaultPaymentMethods(): Promise<PaymentMethod[]> {
    const rawPaymentMethods = await this.request<any[]>(
      "/payment-methods/create-defaults",
      {
        method: "POST",
      },
    );
  ...
  async getExpenseAnalytics(
    ...
    const queryParams = new URLSearchParams({
      ...
      include_previous_period: params.include_previous_period.toString(),
    ...
    return this.request<AnalyticsResponse>(
      `/analytics/expenses?${queryParams}`,
    );
  }

  async exportExpenseAnalytics(
    params: AnalyticsRequest,
    format: "csv" = "csv",
  ): Promise<Blob> {
    const queryParams = new URLSearchParams({
    ...
    const url = `${this.baseURL}/analytics/expenses/export?${queryParams}`;
