...
export const useBudgetGroupStore = create<BudgetGroupState>((set, get) => ({
  ...
  fetchBudgetGroups: async () => {
    set({ isLoadingBudgetGroups: true, error: null });
    ...
    try {
      ...
      set({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch current budget groups",
      });
    }
  },
    ...
    try {
      ...
      set({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch budget groups summary",
      });
    }
  },
  ...
  updateBudgetGroup: async (id: string, data: UpdateBudgetGroupRequest) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentBudgetGroup:
          state.currentBudgetGroup?.id === id
            ? { ...state.currentBudgetGroup, ...updatedBudgetGroup }
            : state.currentBudgetGroup,
        isLoading: false,
      }));
  ...
  deleteBudgetGroup: async (id: string) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentBudgetGroup:
          state.currentBudgetGroup?.id === id ? null : state.currentBudgetGroup,
        selectedBudgetGroupId:
          state.selectedBudgetGroupId === id
            ? null
            : state.selectedBudgetGroupId,
        isLoading: false,
      }));
  ...
  getBudgetGroupWithBudgets: async (id: string) => {
    ...
    try {
      const budgetGroupWithBudgets =
        await apiService.getBudgetGroupWithBudgets(id);
      console.log(
        "🔍 API Response from getBudgetGroupWithBudgets:",
        budgetGroupWithBudgets,
      );
      console.log(
        "🔍 Response keys:",
        Object.keys(budgetGroupWithBudgets || {}),
      );
      console.log("🔍 Budgets array:", budgetGroupWithBudgets?.budgets);
      console.log(
        "🔍 Budgets length:",
        budgetGroupWithBudgets?.budgets?.length,
      );
      console.log(
        "🔍 Full response object:",
        JSON.stringify(budgetGroupWithBudgets, null, 2),
      );
      set({
      ...
      set({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch budget group with budgets",
      });
      throw error;
    }
  },
    ...
    try {
      ...
      set({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch budget group summary",
      });
      throw error;
    }
  },
  ...
  generateBudgetsForGroup: async (
    ...
    try {
      ...
      set({
        isLoading: false,
        error:
          error instanceof Error ? error.message : "Failed to generate budgets",
      });
      throw error;
    }
  },
  ...
  bulkUpdateBudgets: async (id: string, data: BulkBudgetsUpdateRequest) => {
    ...
    try {
      ...
      set({
        isLoading: false,
        error:
          error instanceof Error ? error.message : "Failed to update budgets",
      });
      throw error;
    }
  },
  ...
  getBudgetGroupsByPeriod: (periodType: string) => {
    const { budgetGroups } = get();
    return budgetGroups.filter(
      (bg) => bg.periodType === periodType && bg.isActive,
    );
  },
}));
