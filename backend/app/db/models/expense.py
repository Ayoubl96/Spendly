...
class Expense(Base):
    ...
    )  # Legacy: 'cash', 'card', 'bank_transfer', 'other'
    ...
    category = relationship(
        "Category", foreign_keys=[category_id], back_populates="expenses"
    )
    subcategory = relationship(
        "Category", foreign_keys=[subcategory_id], overlaps="subcategory_expenses"
    )
    ...
    attachments = relationship(
        "ExpenseAttachment", back_populates="expense", lazy="dynamic"
    )
    shared_expense_records = relationship(
        "SharedExpense", back_populates="expense", lazy="dynamic"
    )
    ...
    def get_user_share_amount(self, user_id: Any) -> Decimal:
        ...
        if not self.is_shared:
            ...
            return (
                self.amount_in_base_currency_decimal
...
class SharedExpense(Base):
    ...
    )  # Percentage of the total expense (0-100)
    share_amount = Column(
    ...
    def calculate_share_amount(self, total_expense_amount: Decimal) -> Decimal:
        ...
        share_amount = (
            self.share_percentage_decimal / Decimal("100")
...
class ExpenseShare(Base):
    ...
    share_percentage = Column(
        String, nullable=False
    )  # User's percentage of the expense (0-100)
    share_amount = Column(
        String, nullable=False
    )  # Calculated amount based on percentage
    currency = Column(String(3), ForeignKey("currencies.code"), nullable=False)
    ...
    __table_args__ = (
        CheckConstraint(
            "CAST(share_percentage AS NUMERIC) >= 0 AND CAST(share_percentage AS NUMERIC) <= 100",
            name="valid_share_percentage",
        ),
        CheckConstraint(
            "share_type IN ('percentage', 'fixed_amount', 'equal')",
    ...
    def calculate_share_amount(
        self, total_expense_amount: Decimal, total_participants: int = 1
    ) -> Decimal:
        """Calculate share amount based on share type"""
        if self.share_type == "equal":
            # Equal split among all participants
            return (total_expense_amount / Decimal(total_participants)).quantize(
                Decimal("0.01")
            )
    ...
    def update_share_amount(
        ...
        calculated_amount = self.calculate_share_amount(
            total_expense_amount, total_participants
        )
